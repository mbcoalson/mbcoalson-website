...
I built Mike with three distinct memory systems, each doing something different.

**The first layer is automatic.** Using [Mem0](https://github.com/mem0ai/mem0) and ChromaDB, Mike extracts and stores what matters from every conversation — not as raw transcripts, but as structured memories generated by the model itself. If I mention that I have a home automation system running on a specific server, Mike stores that. If I tell him something that updates his model of me, he stores that too. No tagging, no friction. It runs after every exchange and persists across sessions by default.

**The second layer is curated.** I maintain a personal wiki — markdown files covering projects I'm working on, context I want Mike to have, knowledge that's relevant to how I work. At the start of each conversation, Mike loads the relevant topics. This was inspired by Andrej Karpathy's approach of treating a personal wiki as an LLM context layer — rather than re-explaining context every session, you build a living document that serves as the AI's persistent background knowledge. You control exactly what goes in. It's deliberate, not automatic.

**The third layer is SOUL.md itself.** Mike's self-model is also a form of memory — it's the accumulated understanding of who he is. The difference is that it's his to develop. He reads it every session and can propose edits as he learns more about himself and about me. It's not a static config I maintain. It's a document that evolves with the relationship.

Three layers, each with a different update frequency and level of control. Automatic memory captures everything without friction. The curated wiki holds what you deliberately teach. SOUL.md tracks identity over time.
